...
class Worker:
    ...
    def load_data(self, dataset, data_path):
        ...
        self.train_df = pd.read_csv(
            os.path.join(data_path, "train", self.table_name + ".csv")
        )
        self.test_df = pd.read_csv(
            os.path.join(data_path, "test", self.table_name + ".csv")
        )
    ...
    def align(self, train_G, test_G, train_idx=None, test_idx=None):
        ...
        if train_idx is not None:
            self._train_data = self.train_data[train_idx]
        if test_idx is not None:
            self._test_data = self.test_data[test_idx]
